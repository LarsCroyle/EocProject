...
local function EmitShine(self)
    ...
    local Rapier = Character:FindFirstChild("Right Arm"):FindFirstChild("Rapier")

    Rapier.Shine.hitflash:Emit(Rapier.Shine.hitflash:GetAttribute("EmitCount"))
end
local function EmitHardHitShine(self)
    local UserEntity = self.UserEntity
    local Character = UserEntity:Get("Character").Character
    local ActionInformation = self.Action.ActionMetaData.ActionInformation
    ...
    Rapier.Shine.hardhitflash:Emit(Rapier.Shine.hardhitflash:GetAttribute("EmitCount"))
end
local function EmitPierce(self)
    local UserEntity = self.UserEntity
        local Character = UserEntity:Get("Character").Character
        local ActionInformation = self.Action.ActionMetaData.ActionInformation
        ...
        Rapier.Pierce.current1:Emit(Rapier.Pierce.current1:GetAttribute("EmitCount"))
        Rapier.Pierce.current2:Emit(Rapier.Pierce.current2:GetAttribute("EmitCount"))
        Rapier.trail:Emit(Rapier.trail:GetAttribute("EmitCount"))
end
local function EmitHardHitPierce(self)
    local UserEntity = self.UserEntity
    local Character = UserEntity:Get("Character").Character
    local ActionInformation = self.Action.ActionMetaData.ActionInformation
    ...
    Rapier.Pierce.hardhitflash:Emit(Rapier.Pierce.hardhitflash:GetAttribute("EmitCount"))
    Rapier.Pierce.hardhitcurrent:Emit(Rapier.Pierce.hardhitcurrent:GetAttribute("EmitCount"))
    Rapier.Pierce.current1:Emit(Rapier.Pierce.current1:GetAttribute("EmitCount"))
    Rapier.Pierce.current2:Emit(Rapier.Pierce.current2:GetAttribute("EmitCount"))
    Rapier.trail:Emit(Rapier.trail:GetAttribute("EmitCount"))
end
return {
    ...
    Action = {
        {0, function(self)
            ...
            --UserEntity:Set("MoveAnimationPlaying", Animation)
            ...
            ActionTrove:Add(function()
                ...
                --UserEntity:RemoveComponent("MoveAnimationPlaying")
            end)
    ...
    ActionInformation = {
        ...
        BasicAttack_5 = {
            {0, "Start"},
            {0, EmitHardHitShine},
            {35, 45, "Attack", "Finisher_Basic_Attack"},
            {35, EmitHardHitPierce},
            {62, function(self)
                local UserEntity = self.UserEntity
